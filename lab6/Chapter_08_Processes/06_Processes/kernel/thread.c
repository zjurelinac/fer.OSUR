				...
				int save_old_state )
{
	...
	LOG(DBG, " Creating new thread <%d> :: stack_addr = %x, process = %s", kthread->id, stack, kproc->name);
...
void *kthread_get_context ( kthread_t *kthread )
{
	...
	if ( kthread )
		ret = &kthread->state.context;
	else
		ret = &active_thread->state.context;

	if(kernel_scheduler_debug_mode)
		LOG(DBG, " Context ESP = %x, proc = %x", ((context_t*) ret)->context.esp, ((context_t*) ret)->proc);
	return ret;*/
	if ( kthread )
...
int kthread_info ()
{
	...
	kprintf ( "[this]\tid=%d (desc. at %x) in process at %x, size=%d, end=%x\n",
		  active_thread->id, active_thread, active_thread->proc->m.start,
		  active_thread->proc->m.size, active_thread->proc->m.start + active_thread->proc->m.size );
	...
	context = kthread_get_context(NULL);
	kprintf( "\tESP=%x, SS=%x\n", context->context.esp, context->context.ss);

	kthread = list_get ( &all_threads, FIRST );
	while ( kthread )
	{
		kprintf ( "[%d]\tid=%d (desc. at %x) in process at %x, size=%d, end=%x\n",
			 i++, kthread->id, kthread, kthread->proc->m.start,
			 kthread->proc->m.size, kthread->proc->m.start + kthread->proc->m.size );
