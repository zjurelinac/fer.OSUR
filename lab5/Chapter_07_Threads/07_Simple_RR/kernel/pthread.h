...
/*! rw lock */
typedef struct _pthread_rwlock_t_
{
	id_t       id;
		   /* system level id */

	kthread_t  *wr_owner;
		   /* write owner thread descriptor, or NULL if rwlock not locked for writing */
	...
	kthread_q   rd_queue;
		    /* queue for blocked reader threads */

	kthread_q 	wr_queue;
			/* queue for blocked writer threads */
} kpthread_rwlock_t;
