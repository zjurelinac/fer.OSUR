...
int kthreadq_find(kthread_q *q, kthread_t *kthread){
	ASSERT(q && kthread);
	return list_find(&q->q, &kthread->list) != NULL;
}
void kthreadq_dump(kthread_q *q){
	kthread_t *kthread;
	ASSERT(q);
	kthread = kthreadq_get(q);
	while(kthread != NULL){
		kprintf("<Thread %d>, ", kthread->id);
		kthread = kthreadq_get_next(kthread);
	}
}
