...
static void kthread_queue_dump(queue_t *queue){
	item_t *tmp = queue->first;
	kprintf("[");
	while(tmp != NULL){
		kprintf("<Thread %d>, ", kthread_get_id((kthread_t*) tmp->value));
		tmp = tmp->next;
	}
	kprintf("NULL];\n");
}

int sys__pthread_rwlock_init(pthread_rwlock_t *rwlock, const pthread_rwlockattr_t *attr){
	...
	krwlock->id = k_new_id();
	krwlock->wr_owner = NULL;
	krwlock->ref_cnt = 1;

	queue_init(&krwlock->rd_owners);
	kthreadq_init ( &krwlock->rd_queue );
	kthreadq_init ( &krwlock->wr_queue );
...
int sys__pthread_rwlock_destroy(pthread_rwlock_t *rwlock){
	...
	ASSERT_ERRNO_AND_EXIT (
		krwlock->wr_owner == NULL &&
		queue_empty(&krwlock->rd_owners) &&
		kthreadq_get(&krwlock->rd_queue) == NULL &&
		kthreadq_get(&krwlock->wr_queue) == NULL,
		ENOTEMPTY
	);
...
static int rwlock_rdlock ( kpthread_rwlock_t *krwlock, kthread_t *kthread );
static int rwlock_wrlock ( kpthread_rwlock_t *krwlock, kthread_t *kthread );
...
int sys__pthread_rwlock_rdlock(pthread_rwlock_t *rwlock){
	...
	retval = rwlock_rdlock ( krwlock, kthread_get_active () );
...
int sys__pthread_rwlock_wrlock(pthread_rwlock_t *rwlock){
	...
	retval = rwlock_wrlock ( krwlock, kthread_get_active () );
...
/*! lock rwlock for reading; return 0 if locked, 1 if thread blocked, -1 if error */
static int rwlock_rdlock ( kpthread_rwlock_t *krwlock, kthread_t *kthread ){

	// If not write-locked and no writer blocked
	if(krwlock->wr_owner == NULL && kthreadq_get(&krwlock->wr_queue) == NULL){
		// Then lock for reading, add tread to owners

		// But first, check if already locked by the same thread
		if(queue_find(&krwlock->rd_owners, kthread) != NULL){
			kthread_set_errno ( kthread, EDEADLK );
			return -1;
		}

		queue_push(&krwlock->rd_owners, kthread);
		kthread_set_errno ( kthread, EXIT_SUCCESS );

	#ifdef DEBUG
		LOG(INFO, "Acquired reading lock - <Thread %d>", kthread_get_id(kthread));
		kprintf("WR = <Thread %d>; RD = ", krwlock->wr_owner ? kthread_get_id(krwlock->wr_owner) : -1);
		kthread_queue_dump(&krwlock->rd_owners);
	#endif

		return 0;
	} else {
		// Already locked for writing
		if(krwlock->wr_owner == kthread){ // Also locked by the same thread
			kthread_set_errno ( kthread, EDEADLK );
			return -1;
		}

		kthread_set_errno ( kthread, EXIT_SUCCESS );
		kthread_enqueue ( kthread, &krwlock->rd_queue, 0, NULL, NULL );
		LOG(INFO, "Blocked <Thread %d> for reading later", kthread_get_id(kthread));
...
/*! lock rwlock for writing; return 0 if locked, 1 if thread blocked, -1 if error */
static int rwlock_wrlock(kpthread_rwlock_t *krwlock, kthread_t *kthread){
	if(krwlock->wr_owner == NULL && queue_empty(&krwlock->rd_owners)){

		krwlock->wr_owner = kthread;
		kthread_set_errno ( kthread, EXIT_SUCCESS );

	#ifdef DEBUG
		LOG(INFO, "Acquired writing lock - <Thread %d>", kthread_get_id(kthread));
		kprintf("WR = <Thread %d>; RD = ", krwlock->wr_owner ? kthread_get_id(krwlock->wr_owner) : -1);
		kthread_queue_dump(&krwlock->rd_owners);
	#endif
		...
		kthread_set_errno ( kthread, EXIT_SUCCESS );
		kthread_enqueue ( kthread, &krwlock->wr_queue, 0, NULL, NULL );
		LOG(INFO, "Blocked <Thread %d> for writing later", kthread_get_id(kthread));
...
int sys__pthread_rwlock_unlock(pthread_rwlock_t *rwlock){
	kpthread_rwlock_t *krwlock;
	kobject_t *kobj;
	kthread_t *blocked, *active = kthread_get_active();
	...
	ASSERT_ERRNO_AND_EXIT ( list_find ( &kobjects, &kobj->list ),
				EINVAL );
	krwlock = kobj->kobject;
	ASSERT_ERRNO_AND_EXIT ( krwlock && krwlock->id == krwlock->id, EINVAL );

	if(krwlock->wr_owner == active){ // This thread locked it for writing

		LOG(INFO, ">> Releasing write lock");
		SET_ERRNO ( EXIT_SUCCESS );

		// Check if there is another writer blocked
		krwlock->wr_owner = kthreadq_get(&krwlock->wr_queue);
		if(krwlock->wr_owner != NULL){

	#ifdef DEBUG
			LOG(INFO, "Releasing blocked writer <Thread %d>", kthread_get_id(krwlock->wr_owner));
			kprintf("WR = <Thread %d>; RD = ", krwlock->wr_owner ? kthread_get_id(krwlock->wr_owner) : -1);
			kthread_queue_dump(&krwlock->rd_owners);
	#endif

			kthreadq_release(&krwlock->wr_queue);
			kthreads_schedule();

		} else { // Release all blocked readers

			blocked = kthreadq_get(&krwlock->rd_queue);
			while(blocked != NULL){
				LOG(INFO, "Releasing blocked reader <Thread %d>", kthread_get_id(blocked));
				queue_push(&krwlock->rd_owners, blocked);
				blocked = kthreadq_get_next(blocked);
			}
	#ifdef DEBUG
			kprintf("WR = <Thread %d>; RD = ", krwlock->wr_owner ? kthread_get_id(krwlock->wr_owner) : -1);
			kthread_queue_dump(&krwlock->rd_owners);
	#endif
			kthreadq_release_all( &krwlock->rd_queue );
			kthreads_schedule();
	...
	} else if(queue_find(&krwlock->rd_owners, active)){ // This thread locked it for reading

		LOG(INFO, ">> Releasing read lock");
		SET_ERRNO ( EXIT_SUCCESS );

		queue_remove(&krwlock->rd_owners, active);

		if(queue_empty(&krwlock->rd_owners)){
			krwlock->wr_owner = kthreadq_get(&krwlock->wr_queue);
			if(krwlock->wr_owner != NULL){
		...
		LOG(ERROR, "Something not right with unlocking");
